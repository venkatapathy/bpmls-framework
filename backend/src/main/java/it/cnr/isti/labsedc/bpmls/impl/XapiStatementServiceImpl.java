...
public class XapiStatementServiceImpl implements XapiStatementService {
	...
	public void spawnAndTryPublishLPStatements(String umailbox,Verb verb,LearningPathInstance lpId){
		try {
			...
			try {
				String publishedId= client.postStatement(statement);
				if(publishedId != null){
					LearningStatementRef statementRef=new LearningStatementRef(publishedId, Integer.toString(lpId.getLpInstId()), LearningStatementRef.LearningTypeEnum.LEARNING_PATH);
			...
			// catching any exception because not gonna affect the application for this
	...
	public void spawnAndTryPublishLSStatements(String umailbox,Verb verb,LearningScenarioInstance lsInstance){
		try {
			...
			try {
				//get the parent 
				List<LearningStatementRef> statementRefs=learningStatementRepo.findByInstanceIdAndLearningType(Integer.toString(lsInstance.getLpInstance().getLpInstId()), LearningStatementRef.LearningTypeEnum.LEARNING_PATH);
				...
				String publishedId= client.postStatement(statement);
				if(publishedId != null){
					LearningStatementRef statementRef=new LearningStatementRef(publishedId, Integer.toString(lsInstance.getLsInstId()), LearningStatementRef.LearningTypeEnum.LEARNING_SCENARIO);
					learningStatementRepo.saveAndFlush(statementRef);
				}
	...
	public void spawnAndTryPublishTaskStatements(String umailbox,Verb verb,DelegateTask task,LearningScenarioInstance lsInstance){
		try {
			...
			try {
				//get the parent 
				List<LearningStatementRef> statementRefs=learningStatementRepo.findByInstanceIdAndLearningType(Integer.toString(lsInstance.getLsInstId()), LearningStatementRef.LearningTypeEnum.LEARNING_SCENARIO);
