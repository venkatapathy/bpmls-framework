...
import it.cnr.isti.labsedc.bpmls.impl.XapiStatementServiceImpl;
...
/**
 * Main interfact of the Learning Process Engine which contains within itself
...
public interface LearningProcessEngine {
	 ...
	 * 
	 * public LearningScenarioInstance startLearningScenario(LearningScenario
	 * learningScenario);
	 * 
	 * public LearningScenario getLearningScenario(String learningScenarioId);
	 * 
	 * public LearningScenarioInstance getLearningScenarioInstance(String
	 * learningScenarioInstanceId);
	 * 
	 * public Task getCurrentLearningTask(LearningScenarioInstance
	 * learningScenarioInstance);
	 * 
	 * public void completeLearningTask(LearningScenarioInstance
	 * learningScenarioInstance, List<DataObject> userSubmittedDOs);
	 * 
	 */
	...
	// /**
	// * This will get the current learning task of the current learning
	// scenario
	// *
	...
	// LPInstance. null if there are no task
	// *
	// */
	...
	public class ThirdEyeTaskListener extends ReactorTaskListener {
		...
		public void notify(DelegateTask delegateTask) {
			// need to flush the jpa repo because this might be called even
			// before lsinst set

			// when a task is created update the oracle values to its
			// corresponding task
			...
			try {
				List<DataObject> curOracleDOs = lpRepoService.getCurrentOracleValuesFromRepo(
						lsInst.getProcessInstanceId(), lsInst.getLsId(), delegateTask.getTaskDefinitionKey());
				if (curOracleDOs != null) {
					logger.info(
							"Updating oracle values for the Learning Scenario: " + lsInst.getLsId() + " with InstId: "
									+ lsInst.getLsInstId() + ". For the task: " + delegateTask.getTaskDefinitionKey());
			...
			try {
				xApiService.spawnAndTryPublishTaskStatements(lsInst.getLpInstance().getLdInstance().getUsername(),
						Verbs.launched(), delegateTask, lsInst);
	...
	public class ThirdEyeExecutionListener extends ReactorExecutionListener {
		...
		public void notify(DelegateExecution delegateExecution) {
			...
			// when a task is created update the oracle values to its
			// corresponding task
			LearningScenarioInstance lsInst = lsJpaRepo
					.findOneByProcessInstanceId(delegateExecution.getProcessInstanceId());
			if (lsInst == null) {
				return;
			}

			logger.info("Process completed for the LearningScenario for: " + lsInst.getLsId() + ". With instId: "
					+ lsInst.getLsInstId());
			logger.info("Changing status from: " + LearningScenarioEvents.LS_STATUS_RUNNING + " to "
					+ LearningScenarioEvents.LS_STATUS_COMPLETED);

			lsInst.setStatus(LearningScenarioEvents.LS_STATUS_COMPLETED);
			lsJpaRepo.saveAndFlush(lsInst);

			// xAPI Event
			// learningscenario end
			// before returning try ans spwan xapi statement, will not affect
			// the flow
			try {
				xApiService.spawnAndTryPublishLSStatements(lsInst.getLpInstance().getLdInstance().getUsername(),
						Verbs.completed(), lsInst);
